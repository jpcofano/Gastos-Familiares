// Prompt de extraccion de posiciones (Paso 1 del wizard de "Actualizar posiciones").
//
// UNICA COPIA. Antes vivia en docs/patrimonio/patrimonio-extraccion.md, que ahora
// solo apunta aca. Dos copias divergen: ya paso con el seed de ACN, el de Efectivo
// y el de Pionero.
//
// El texto esta calibrado contra los resumenes reales de PPI, Balanz, Nexo y
// Bitfinex. Cualquier cambio de contenido es un spec aparte: una "mejora" de
// redaccion puede romper la extraccion sin que se note hasta la proxima corrida.
// Lo unico que la app sustituye son los dos marcadores de la cabecera.

const PLANTILLA = `# Prompt de extracción — Patrimonio
...
TC_REFERENCIA_ARS_USD: {{TC_REFERENCIA}}   # informativo; la app usa su propio tcDiario
\`\`\`
...
/** Fecha de hoy en zona Argentina, formato YYYY-MM-DD. */
export function hoyArgentinaISO(): string {
  return new Date().toLocaleDateString('en-CA', { timeZone: 'America/Argentina/Buenos_Aires' });
}

/** Placeholder del TC: no se prellena a proposito (ver promptExtraccionPosiciones). */
export const TC_REFERENCIA_PLACEHOLDER = '<completar a mano: MEP del dia>';

/**
 * El prompt listo para pegar en un chat.
 *
 * FECHA_CORRIDA viene prellenada. TC_REFERENCIA no: es informativo y la app usa su
 * propio tcDiario, asi que poner un numero ahi invitaria a creer que ese valor entra
 * al calculo.
 */
export function promptExtraccionPosiciones(fechaCorrida: string = hoyArgentinaISO()): string {
  return PLANTILLA
    .replace('{{FECHA_CORRIDA}}', fechaCorrida)
    .replace('{{TC_REFERENCIA}}', TC_REFERENCIA_PLACEHOLDER);
}
